...
**Next:** Step 10.2 — Resolver wiring. Register a `ConflictResolver` on the app's sync-engine instance that opens `ConflictModal` and awaits the user's choice. Multiple conflicts queue and are presented one at a time. Output `useConflictResolver` hook + connection in app shell. Done when a synthetic two-conflict pull opens the modal twice in sequence, and choosing remote leaves the same record on both sides after sync.
