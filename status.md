...
**Next:** Step 9.1 — Options shell + sub-routing (opens Phase 9). `/options` lists groups; `/options/:group` opens the corresponding panel; back-button friendly. Outputs `OptionsView.tsx`, `OptionsList.tsx`. Done when browser back/forward works between groups.
