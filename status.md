...
**Next:** Step 9.2 — Backends panel. List registered backends with sync status, connect/disconnect actions, and a "default backend for new tasks" radio. Output `BackendsPanel.tsx`. Done when default selection persists and UI shows last-sync timestamp from the sync engine.
