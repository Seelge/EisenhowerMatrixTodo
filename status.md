...
**Next:** Step 10.3 — Backstop for non-modal conflicts. If the user is mid-action (drag, composing) when a conflict arrives, queue it; show a small badge on the sync-status icon; the modal opens on next user idle or on click. Output: idle detector + queue. Done when, during an active drag, no modal appears; drag completes; modal opens.
