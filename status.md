...
**Next:** Step 9.3 — Account panel. Show connected identity per backend; sign-out per backend. Local backend has no account; renders informational copy. Google/MS rows are placeholders. Output `AccountPanel.tsx`. Done when the UI renders correctly for the local-only state.
