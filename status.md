...
**Next:** Step 10.1 — Conflict modal (opens Phase 10). Modal displaying local vs remote whole-record side by side with the differing fields highlighted; "Keep local" / "Keep remote" actions. Output `ConflictModal.tsx`. Done when opening with a synthetic `ConflictRecord` shows both sides and allows choosing.
